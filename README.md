...
HDRP 를 사용하다보니 아무거나 옵션 추가해서 그래픽을 좋게하면 렉이 굉장히 걸린다. 심지어 RTX 3080에서도 프레임이 20~40프레임이 나오는정도..  
특히 터레인을 꾸밀때 렉이 심하다. 터레인에 기본적으로 자연의 형태를 뛰운다면 GRASS 나 TREE 같은 오브젝트를 많이 설치해야한다. (오픈월드 기준)  
현재 프로젝트의 GRASS 와 TREE 의 오브젝트 갯수는 약 34만개 정도로 본디 전에는 이런식으로 깔면 렉이 굉장히 심했는데,  
현재는 렉이 없다. 최적화 방안은 우선 프로파일러를 돌렸다. 프로파일러를 돌리니 코드상의 문제는 없고 90프로가 유니티 엔진에서 프레임 드랍이 생기는것  
즉 유니티 엔진에서만 리소스를 잡아 먹는 다는 것인데, 역시 GRASS 나 TREE와 같은 오브젝트들이 원인이다. 해당 원인 해결 방안은 첫번째로 LOD 설정이다.   
나무와 같은 오브젝트에는 LOD 컴포넌트가 붙어 있는데 LOD로 3 LVL를 낮게하면 멀리 있으면 형질이 뭉게지는 대신 프레임이 많이 상승한다  
그리고 터레인 옵션중에 DISTANCE FADE를 사용하는 것이다. 멀리 있는 물체일 수록 검정색 혹은 자신이 원하는 색으로 안보이게 하는대신, 육안으로 보기는 안좋다.
현재 DISTANCE FADE로 조절을 해서 육안으로 보기 안좋긴한데, 월드를 수정해서 오브젝트 갯수를 줄일 예정이다.
